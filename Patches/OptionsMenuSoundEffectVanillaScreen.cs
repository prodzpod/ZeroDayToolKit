...
namespace ZeroDayToolKit.Patches
{
    ...
    public class OptionsMenuSoundEffectVanillaScreen
    {
        internal static void ILManipulator(ILContext il)
        {
            ...
            c.Emit(OpCodes.Newobj, AccessTools.Constructor(typeof(Vector2), [typeof(float), typeof(float)]));
            c.Emit(OpCodes.Ldstr, "Sound Effects Volume");
            c.Emit(OpCodes.Call, AccessTools.Method(typeof(LocaleTerms), nameof(LocaleTerms.Loc), [typeof(string)]));
            c.Emit(OpCodes.Ldloca, 10);
            c.Emit(OpCodes.Initobj, typeof(Color?));
            c.Emit(OpCodes.Ldloc, 10);
            c.Emit(OpCodes.Ldloc, 4);
            c.Emit(OpCodes.Call, AccessTools.Method(typeof(TextItem), nameof(TextItem.doLabel), [typeof(Vector2), typeof(string), typeof(Color?), typeof(float)]));
            c.Emit(OpCodes.Nop);
            ...
            c.Emit(OpCodes.Call, AccessTools.Method(typeof(SliderBar), nameof(SliderBar.doSliderBar), [typeof(int), typeof(int), typeof(int), typeof(int), typeof(int), typeof(float), typeof(float), typeof(float), typeof(float)]));
